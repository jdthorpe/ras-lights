...
import { list_library, upsert_library, remove_library } from "./src/lib";
...
async function process_args() {
    ...
    switch (args.command) {
        case "library":
            switch (args.verb) {
                case "list":
                    const libs = await list_library();
                    libs.forEach((lib) => console.log(JSON.stringify(lib)));
                    console.log("ok");
                    break;
                case "delete":
                    await remove_library(args.name);
                    console.log("ok");
                    break;
                case "create":
                    await upsert_library({ name: args.name, path: args.path });
                    console.log("ok");
                    break;
                default:
